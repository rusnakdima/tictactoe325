...
socket.on('dataStats', (stats) => {
    ...
    for(var i = 0; i < Object.keys(stats).length; i++){
        ...
        $(td).text(Object.keys(stats)[i]);
        tr.append(td);
        var obj = stats[Object.keys(stats)[i]];
        for(var j = 0; j < Object.keys(obj).length; j++){
            if(document.cookie.match(/nickname=(.+?)(;|$)/)[1] == Object.keys(stats)[i]){
                document.cookie = "winnings="+obj['winnings'];
                document.cookie = "draws="+obj['draws'];
                document.cookie = "losses="+obj['losses'];
            }
            var td = $("<td class='border-2 border-slate-400 px-2 w-1/6'></td>");
            $(td).text(Object.values(obj)[j]);
            tr.append(td);
        }
        tableStats.append(tr);
    }
